...
def random_gains(rgb_gain_ratio=1.0, red_gain_range=[1.9, 2.4], blue_gain_range=[1.5, 1.9]):
    ...
    red_gain  =  torch.FloatTensor(1).uniform_(red_gain_range[0], red_gain_range[1])
    blue_gain =  torch.FloatTensor(1).uniform_(blue_gain_range[0], blue_gain_range[1])
...
def add_noise(image, shot_noise=0.01, read_noise=0.0005, read_noise_exponent=2):
...
def unprocess_meta_gt(image, rgb_gains, red_gains, blue_gains, rgb2cam, cam2rgb):
    """Unprocesses an image from sRGB to realistic raw data."""
...
def add_noise_test(image, shot_noise=0.01, read_noise=0.0005, count=0):
    ...
    noise = torch.normal(mean=torch.zeros_like(variance), std=torch.sqrt(variance), generator=seed)
    out      = image + noise
    out      = out.permute(2, 0, 1) # Re-Permute the tensor back to CxHxW format
    return out